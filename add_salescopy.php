...
if(isset($_POST['payment'])){
			...
			$count = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE sku='$sku'");
			...
			$db->query("insert into stock_sales (sku,tax,tax_dis,discount,dis_amount,grand_total,transactionid,stock_name,selling_price,quantity,amount,date,username,customer_id,subtotal,payment,balance,due,mode,description,count1,billnumber) 
                            values('$sku',$tax,'$tax_dis',$discount,$dis_amount,$payable,'$autoid','$name1',$rate,$quantity,$total,'$mysqldate','$username','$customer',$subtotal,$payment,$balance,'$due','$mode','$description',$i+1,'$bill_no')");
			
			$amount = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE sku='$sku'");
				$amount1 = $amount - $quantity;
			
			$db->query("insert into stock_entries (sku,stock_id,stock_name,quantity,opening_stock,closing_stock,date,username,type,salesid,total,selling_price,count1,billnumber) values('$sku','$autoid','$name1',$quantity,$amount,$amount1,'$mysqldate','$username','sales','$autoid',$total,$rate,$i+1,'$bill_no')");
			//echo "<br><font color=green size=+1 >New Sales Added ! Transaction ID [ $autoid ]</font>" ;
			
			
			$amount = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE sku='$sku'");
				$amount1 = $amount - $quantity;
				$db->execute("UPDATE stock_avail SET quantity=$amount1 WHERE sku='$sku'");
			
			}
			else 
			{
				echo "<br><font color=green size=+1 >There is no enough 
        ...
        $("#sku").blur(function()
			{
			 ...
			 //alert($("#supplier").val() );	
			 $.post('check_item_details.php', {supplier_name:$("#supplier").val(),stock_name1: $(this).val() },
				function(data){
                                                              	$("#mrp").val(data.sell);
								$("#sell").val(data.sell);
...
function numbersonly(e){
    ...
    $('<tr id='+item+'><td><lable id='+item+'roll class=jibi007 >'+roll+'</label></td><td><input type=hidden value='+item+' id='+item+'id ><input type=text name="sku[]"  value='+sku+' id='+item+'sk style="width: 150px" class="round  my_with" readonly="readonly" ></td><td><input type=text name="stock_name[]"  id='+item+'st style="width: 150px" class="round  my_with" readonly="readonly" ></td><td><input type=text name=quty[] readonly="readonly" value='+quty+' id='+item+'q class="round  my_with" style="text-align:right;" ></td><td><input type=text name=sell[] readonly="readonly" value='+sell+' id='+item+'s class="round  my_with" style="text-align:right;"  ></td><td><input type=text name=stock[] readonly="readonly" value='+disc+' id='+item+'p class="round  my_with" style="text-align:right;" ></td><td><input type=text name=jibi[] readonly="readonly" value='+total+' id='+item+'to class="round  my_with" style="width: 120px;margin-left:20px;text-align:right;" ><input type=hidden name=total[] id='+item+'my_tot value='+main_total+'> </td><td><input type=button value="" id='+item+' style="width:30px;border:none;height:30px;background:url(images/edit_new.png)" class="round" onclick="edit_stock_details(this.id)"  ></td><td><input type=button value="" id='+item+' style="width:30px;border:none;height:30px;background:url(images/close_new.png)" class="round" onclick=reduce_balance("'+item+'");$(this).closest("tr").remove(); ></td></tr>').fadeIn("slow").appendTo('#item_copy_final');
    ...
    document.getElementById(id+'sk').value=document.getElementById('sku').value;  
   ...
   function edit_stock_details(id) {
     ...
     document.getElementById('sku').value=document.getElementById(id+'sk').value;
...
// Added hidden field to save payable_amount for tax
      document.getElementById('payable_amount_hidden').value=parseFloat(document.getElementById('grand_total').value)-discont;    
...
function tax_amount(){ 
        if(document.getElementById('grand_total').value!=""){
        	// Taking hidden field payable_amount for tax
           //tax1=parseFloat(document.getElementById('payable_amount').value)*(parseFloat(document.getElementById('tax').value))/100;
        	tax1=parseFloat(document.getElementById('payable_amount_hidden').value)*(parseFloat(document.getElementById('tax').value))/100;

        }
        
    //document.getElementById('payable_amount').value=parseFloat(document.getElementById('payable_amount').value)+tax1;
	 document.getElementById('payable_amount').value=parseFloat(document.getElementById('payable_amount_hidden').value)+tax1;
...
<link rel="stylesheet"  href="dist/js/jquery.alerts.css" >
			  ...
			  <td>Sellin Price:<br>(Edit)</td>
                        ...
                        <tr>
                 		<td><input name=""  type="text" id="sku" maxlength="200"  class="round default-width-input " style="width: 150px"   /></td>
			...
			<td><input name=""  type="text" id="stock" readonly="" maxlength="200"  class="round  my_with" /></td>
                       	...
                       	<td><input  name=""  type="text" id="mrp" readonly="" maxlength="200"  class="round default-width-input my_with"   /></td>
	
			<td><input  name=""  type="text" id="sell"   maxlength="200"  class="round default-width-input my_with"   /></td>

			<td><input name=""  type="text" id="quty"  maxlength="200"   class="round default-width-input my_with" onKeyPress="quantity_chnage(event);return numbersonly(event)" onkeyup="total_amount();unique_check();stock_size();"    /></td> 
                  ...
                  <td><textarea name="description"></textarea></td>
                    ...
                    <input type="hidden" id="payable_amount_hidden" readonly="readonly" name="payable" class="round default-width-input"  style="text-align:right;width: 120px" >
                 ...
                 <td> Tax:<input type="text" maxlength="5" onkeyup=" tax_amount(); " onkeypress="return numbersonly(event);" name="tax" id="tax" ></td>              
               <td>P.O.No:<input type="text" name="tax_dis"> </td>
