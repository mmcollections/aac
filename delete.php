...
else
{
	...
	//Code to delete stock_avail record based on Stock_ID
	if($tablename=="stock_details")
	{
	$sql = "SELECT  stock_avail.id from stock_avail,stock_details where stock_details.id=$id and	 stock_details.sku = stock_avail.sku";
	$stock_avail_id = mysql_fetch_row(mysql_query($sql));
	//print_r($stock_avail_id);exit;
	$sql = "DELETE FROM stock_avail WHERE id=".$stock_avail_id[0];
	$db->execute($sql);
	}	
